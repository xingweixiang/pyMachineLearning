...
### 6、关系图算法
- 关系图算法可以通过算法挖掘关系网络中的主要成分、预测行车轨迹的最短路径、判断社交网络中彼此的亲密程度、做金融行业的风险管控等。
- 运用较多的是标签传播（LPA）和迪杰斯特拉最短路径的算法。
- LPA 是半监督机器学习算法，基本原理是在一个大数据集中，已知其中一部分的标记数据，然后通过数据间彼此的联系，最后得到全部数据的标记结果。如金融风控
### 六、深度学习
### 1、深度神经网络
- 深度神经网络（DNN）：指多层次的神经网络，模型的隐藏层之间彼此相连。针对处理数据的种类和特点不同，衍生出各种不同的结构。如CNN和RNN
### 2、卷积神经网络
- 卷积神经网络（CNN）：是一种比较特殊的深度学习结构，主要通过卷积来解决空间上一些复杂特征的问题。<br>
典型的就是图片识别问题，像车牌识别、手写字识别和人脸识别的模型训练都是依赖于CNN。
### 3、循环神经网络
- 循环神经网络（RNN）：是一种环状的深度神经网络，常用来解决时序行为的问题。可应用于自然语言理解和股票预测等相关场景。
